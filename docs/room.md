...
- 这条消息里**没有** `pai` 字段
- `tehais` — **MJAI 拓展**：和了者亮手牌（闭张最多 13 张），其余座位为 `"?"`。`"?"` 表示本事件不亮该座位，不是「没手牌」；若你自己没亮，格子也会是 `"?"`，客户端应继续用本地已有手牌，不要用全 `"?"` 盖掉自己。
...
```json
{
  ...
  "tehais": [
    ["1m","2m","3m","4p","5p","6p","7s","8s","9s","E","E","S","S"],
    ["?","?","?","?","?","?","?","?","?","?","?","?","?"],
    ["1p","1p","1p","2s","3s","4s","5m","6m","7m","W","W","N","N"],
    ["?","?","?","?","?","?","?","?","?","?","?","?","?"]
  ]
}
```
...
- `tehais` — **MJAI 拓展**：荒牌时听牌（或流满）座位亮牌；九种九牌时申报者亮牌；不亮的座位为 `"?"`。途中流局（四风/四杠/四立直/三家和等）不带此字段。同样：你自己没听时，自己那格也会是 `"?"`，只表示不对外亮，客户端勿覆盖本地手牌。
